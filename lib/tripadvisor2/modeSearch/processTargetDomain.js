...
const processTargetDomain = async (
  ...
  try {
    await page.delayRandomMsMove(400, 1000, random(1000, 3000), 35);
    ...
    let oldY = 0;
    for (let index = 0; index < random(1000, 1200); index++) {
      if (new Date().getTime() - timeStart > config.timLive * 1000) break;
      if (randomScore(15))
        await page.delayRandomMsMove(5000, 25000, random(1000, 8000), 35);
      else await page.delayRandomMsMove(500, 4000, random(500, 3000), 20);
      try {
        let current;
        switch (level) {
          case 2:
