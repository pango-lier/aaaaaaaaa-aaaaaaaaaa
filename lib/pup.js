...
class PuppeteerActionFunc {
    ...
    async scrollRandDown({ delayMin = 2, delayMax = 10 }, { stepMin = 50, stepMin = 400 }, { loopMin = 1, loopMax = 3 }) {
        for (let i = 0; i < random(loopMin, loopMax); i++) {
            await this.delayRandom(delayMin, delayMax);
            await this.page.evaluate(() => {
                window.scrollBy(0, random(stepMin, stepMin));
            });
        }
    }

    async scrollRandUp({ delayMin = 2, delayMax = 10 }, { stepMin = 50, stepMin = 400 }, { loopMin = 1, loopMax = 3 }) {
        for (let i = 0; i < random(loopMin, loopMax); i++) {
            await this.delayRandom(delayMin, delayMax);
            await this.page.evaluate(() => {
                window.scrollBy(0, -random(stepMin, stepMin));
