...
class PuppeteerActionFunc {
  ...
  delayTypingTime;
  multiDelay;
  constructor(page, delayClickTime, delayTypingTime, multiDelay = 1) {
    this.page = page;
    this.delayClickTime = delayClickTime || 0.3;
    this.delayTypingTime = delayTypingTime || 0.05;
    this.multiDelay = multiDelay || 1;
  }
  /**
   * click element
   */
  ...
  async uploadImage(pathFiles, fileChooserTriggerXpath) {
  ...
  async uploadImageFrame(frame, pathFiles, fileChooserTriggerXpath) {
    const [fileChooser] = await Promise.all([
      this.page.waitForFileChooser(),
  ...
  async delay(selector) {
    return delay(Number(selector) * this.multiDelay);
  }

  async delayMs(selector) {
    return delayMs(Number(selector) * this.multiDelay);
  }

  async delayRandomMs(min, max) {
    return delayMs(Number(random(min, max)) * this.multiDelay);
  }

  async delayRandom(min, max) {
    return delay(Number(random(min, max)) * this.multiDelay);
  }
  ...
  async scrollRandUpDownScoreRollBack(
    scoreDown,
    { delayMin = 200, delayMax = 4000 },
    { stepMin = 50, stepMax = 400 },
    { loopMin = 1, loopMax = 6 },
    ...
    let old_y = 0;
    for (let i = 0; i < maxLoop; i++) {
      await this.page.mouse.move(random(50, 500), random(100, 300));
      await this.delayRandomMs(delayMin / 2, delayMax / 2);
      let _random = random(stepMin, stepMax);
      if (!randomScore(scoreDown)) _random = -_random;
      this.page.mouse.wheel({ deltaY: _random });
  ...
  async moveRandom(
    ...
    let timeStop = new Date().getTime() + timeOut;
    const size = await this.getSizeViewport();
    do {
      await this.delayRandomMs(50, 500);
      await this.page.mouse.move(random(0, size.w), random(0, size.h), {
        steps: random(5, 50),
      });
      if (randomScore(score)) await this.delayRandomMs(1500, maxDelay);
    } while (new Date().getTime() < timeStop);
    await this.delayRandomMs(300, 700);
    if (selector) {
      await this.mouseMoveCenterElement(selector);
    }
  }

  async delayRandomMsMove(
    min = 1000,
    max = 2000,
    maxDelay = 3000,
    score = 20,
  ...
  async clickRandom(timeOutMin = 100, timeOutMax = 500, tryClick = 1) {
    const hrefIn = await this.location();
    const browser1 = await this.page.browser();
    const pages1 = await browser1.pages();
    const pages1Length = pages1.length;
    let clickSuccess = false;
    let isNewPopupPage = false;
    for (let index = 0; index < tryClick; index++) {
      console.log(`try click ${index}`);
      try {
        await this.moveRandom(random(timeOutMin, timeOutMax));
        await this.delayRandomMs(50, 150);
        if ((await this.UA()) == "mobie") {
          await this.page.tab();
        } else {
          await this.page.mouse.down(options);
      ...
      await this.delayRandomMs(200, 500);
      const currentUrl = await this.location();

      if (currentUrl !== hrefIn) {
        isNewPopupPage = false;
        clickSuccess = true;
        break;
      } else {
        const browser = await this.page.browser();
        const pages = await browser.pages();
        if (pages.length > pages1Length) {
          isNewPopupPage = true;
          clickSuccess = true;
          break;
        }
    ...
    return { clickSuccess, isNewPopupPage };
  }

  async clickAndMove(selector, timeOutMin = 1000, timeOutMax = 6000) {
    try {
      await this.moveRandom(random(timeOutMin, timeOutMax), selector);
      await this.page.hover(selector);
      await this.delayRandomMs(400, 1500);
      if ((await this.UA()) == "mobie") {
        await this.page.tab();
        ...
        await this.page.mouse.down(options);
        await this.delayRandomMs(50, 1000);
        await this.page.mouse.up(options);
      }
    } catch (error) {
      try {
  ...
  async mouseMoveCenterElement(selector) {
    const center = await this.getCenterXY(selector);
    if (center) {
      console.log(
        `mouseMoveCenterElement :x=${center.centerX},:y=${center.centerY}`
      );
      await this.delayRandomMs(50, 500);
      await this.page.mouse.move(center.centerX, center.centerY, {
        steps: random(7, 25),
      });
      await this.delayRandomMs(1500, 2500);
    }
    return center;
  }

  async getCenterXY(selector) {
  ...
  async installMouseHelper() {
    ...
    await installMouse(this.page);
    const browser = await this.page.browser();
    browser.on("targetcreated", async (target) => {
      try {
        if (target.type() === "page") {
          const newPage = await target.page();
          await installMouse(newPage);
        }
      } catch (error) {}
    });
  }
  ...
  async findParentBySelector(child, selector) {
    var elm = document.querySelector(child);
  ...
  async findParentBySelectorAndClick(child, selector) {
    return this.page.evaluate(
      async ({ child, selector }) => {
        ...
        if (document.documentElement.clientWidth < 500) return await cur.tab();
        else return await cur.click();
      },
      { child, selector }
    );
  }

  async findParentLick(child, selector) {
    const center = await this.mouseMoveCenterElement(child);
    await this.delayRandomMs(100, 800);
    // if (center) {
    //   try {
    //     if ((await this.UA()) == "mobie") {
    //       await this.page.tab();
    ...
    //   } catch (error) {
    try {
      console.log("findParentBySelectorAndClick");
      await this.findParentBySelectorAndClick(child, selector);
    } catch (error) {}
    // }
    // }
    await this.delayRandomMs(600, 1200);
  }
}
