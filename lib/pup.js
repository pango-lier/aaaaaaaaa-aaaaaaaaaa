...
class PuppeteerActionFunc {
    ...
    async scrollRandDown({ delayMin = 200, delayMax = 4000 }, { stepMin = 50, stepMax = 400 }, { loopMin = 1, loopMax = 6 }) {
        ...
        for (let i = 0; i < maxLoop; i++) {
            ...
            await this.page.evaluate(({ _random }) => {
                window.scrollBy(0, _random);
            }, { _random });
            console.table({
                name: "scrollRandDown",
                loop: i,
    ...
    async scrollSelectorView(selector) {
        let _delay = (m1, m2) => delayRandomMs(m1, m2);
        await this.page.exposeFunction("_delay", _delay);
