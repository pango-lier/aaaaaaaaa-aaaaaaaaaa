...
const { delay, random, delayMs, delayRandomMs } = require('./until')
...
class PuppeteerActionFunc {
    ...
    constructor(
        ...
        this.page.exposeFunction("delayRandomMs", delayRandomMs);
    }
    /**
     * click element
     */
    ...
    async scrollRandUpDownScoreRollBack(scoreDown, { delayMin = 200, delayMax = 4000 }, { stepMin = 50, stepMax = 400 }, { loopMin = 1, loopMax = 6 }, enableScrollBack = true) {
        ...
        for (let i = 0; i < maxLoop; i++) {
            ...
            if (enableScrollBack) {
                ...
                if (Math.abs(poision.y) < 2) {
                    console.log("------------------Start ");
                    scoreDown = scoreDown > 50 ? scoreDown : 100 - scoreDown;
                }
                if (Math.abs(poision.y) > size.h) {
                    if (body.h <= Math.abs(poision.y) + size.h) {//end-page
