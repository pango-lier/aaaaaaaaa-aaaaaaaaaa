...
class PuppeteerActionFunc {
  ...
  delayClickTime;
  delayTypingTime;
  constructor(page, delayClickTime, delayTypingTime) {
    this.page = page;
  ...
  async moveRandom(timeOut = 2000, maxDelay = 1000, selector = undefined) {
    let timeStop = new Date().getTime() + timeOut;
    const size = await this.getSizeViewport();
    console.log(`moveRandom :w=${size.w},:h=${size.h}`);
    do {
      await this.delayRandomMs(50, 500);
      await this.page.mouse.move(random(0, size.w), random(0, size.h), {
        steps: random(3, 25),
      });
      console.log(`moveRandom :x=${center.centerX},:y=${center.centerY}`);
      if (randomScore(20)) await this.page.delayRandomMs(200, maxDelay);
  ...
  async clickAndMove(selector, min = 1000, max = 6000) {
    try {
      await this.moveRandom(random(min, max), selector);
      await this.page.hover(selector);
      await this.delayRandomMs(400, 1500);
      if ((await this.UA()) == "mobie") {
        await this.page.tab();
