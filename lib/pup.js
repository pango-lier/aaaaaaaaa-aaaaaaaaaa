...
class PuppeteerActionFunc {
  ...
  async moveRandom(
    ...
    do {
      console.log("moveRandom1");
      await this.delayRandomMs(50, 500);
      console.log("moveRandom2");
      await this.page.mouse.move(random(0, size.w), random(0, size.h), {
        steps: random(5, 50),
      });
      console.log("moveRandom3");
      if (randomScore(score)) await this.delayRandomMs(200, maxDelay);
      console.log("moveRandom4");
    } while (new Date().getTime() < timeStop);
    await this.delayRandomMs(500, 700);
    if (selector) {
