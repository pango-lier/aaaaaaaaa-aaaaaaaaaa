...
class GoogleSearch {
  ...
  async typeSearch(keywords) {
    await this.page.goto("https://www.google.com.vn", {
  ...
  async run(keywords, domain, extraDomainStart = [], extraDomainEnd = []) {
    ...
    if (randomScore(10))
      await gotoRandomWeb(this.page, extraDomainStart, 85, random(2, 8));
    ...
    if (randomScore(10))
      await gotoRandomWeb(this.page, extraDomainEnd, 85, random(2, 8));
  ...
  async runGooglePhoto(keywords, domain) {
    ...
    for (let i = 2; i < 6; i++) {
      ...
      if (
        content &&
        ["images", "image", "hình ảnh"].includes(content.toLowerCase())
      ) {
        await this.page.clickAndMove(selector);
        break;
      }
    }
    ...
    for (let p = 0; p < config.maxPage * config.pageRetries; p++) {
      for (let i = 0; i < random(70, 100); i++) {
        try {
          ...
          await this.page.moveRandom(config.delayScroll, random(1000, 3000));
