...
class GoogleSearch {
    ...
    async scrollSlowlyTopToBottomFoundClickNow(domain, page = 2, config = { levelScroll: 3, scoreScrollDown: 90, scoreScrollUp: 90, rateEnableScrollUp: 40, rateEnableScrollDown: 100 }) {
        ...
        for (let i = 0; i < 400; i++) {
            ...
            const info = await this.scrollDownInfo(domain, scrollDown ? config.scoreScrollDown : (100 - config.scoreScrollUp), page, levelScroll);
    ...
    async scrollDownInfo(domain, scoreDown = 85, scrollFast = 3) {
        ...
        const location = await this.page.location();
        if (location.search("https://www.google.com") < 0) {
            try {
                await this.page.page.goBack({ load: true, timeout: random(2700, 3800) });
            } catch (e) {
            }
        }
        let current;
