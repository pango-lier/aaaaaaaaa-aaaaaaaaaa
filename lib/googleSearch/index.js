...
class GoogleSearch {
    ...
    async scrollSlowlyTopToBottomFoundClickNow(domain, page = 2, config = { maxClickNextStep: 1 }) {
        ...
        for (let i = 0; i < 600; i++) {
            const info = await this.scrollDownInfo(domain);
            if (info.domainFounded) {
                domainFoundedAtPage = { page, domainFounded };
            }
            console.log("Step-->> " + step + " : " + i + " .Link Active :" + info.linkActives.length);
            switch (step) {
                default:
                case 'process':
                    if (randomScore(80 - numberLinkClick * 10)) {
    ...
    async scrollDownInfo(domain, scoreDown = 85, scrollFast = 3) {
        ...
        switch (scrollFast) {
            case 1:
                current = await this.page.scrollRandUpDownScore(scoreDown, { delayMin: 200, delayMax: 600 }, { stepMin: 40, stepMax: 60 }, { loopMin: 1, loopMax: 3 });
                break;
            case 2:
                current = await this.page.scrollRandUpDownScore(scoreDown, { delayMin: 200, delayMax: 600 }, { stepMin: 40, stepMax: 60 }, { loopMin: 1, loopMax: 3 });
                break;
            default:
            case 3:
                current = await this.page.scrollRandUpDownScore(scoreDown, { delayMin: 200, delayMax: 1000 }, { stepMin: 40, stepMax: 60 }, { loopMin: 1, loopMax: 3 });
                break;
            case 4:
            ...
            case 6:
                ...
                break;
            case 7:
                current = await this.page.scrollRandUpDownScore(scoreDown, { delayMin: 200, delayMax: 1600 }, { stepMin: 10, stepMax: 400 }, { loopMin: 1, loopMax: 1 });
                break;
            case 8:
                current = await this.page.scrollRandUpDownScore(scoreDown, { delayMin: 200, delayMax: 1600 }, { stepMin: 10, stepMax: 450 }, { loopMin: 1, loopMax: 1 });
                break;
            case 9:
                current = await this.page.scrollRandUpDownScore(scoreDown, { delayMin: 200, delayMax: 1600 }, { stepMin: 10, stepMax: 500 }, { loopMin: 1, loopMax: 1 });
                break;
            case 10:
                current = await this.page.scrollRandUpDownScore(scoreDown, { delayMin: 200, delayMax: 1600 }, { stepMin: 10, stepMax: 550 }, { loopMin: 1, loopMax: 1 });
                break;
        }
