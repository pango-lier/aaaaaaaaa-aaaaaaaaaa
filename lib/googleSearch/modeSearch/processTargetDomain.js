...
const processTargetDomain = async (page, link, level = 5, scoreRollDow = 85, config) => {
    try {
        ...
        for (let index = 0; index < random(25, 50); index++) {
            let current;
            switch (level) {
                case 2:
                ...
                case 5:
                    ...
                    break;
                case 6:
                    current = await page.scrollRandUpDownScoreRollBack(80, { delayMin: 400, delayMax: 3500 }, { stepMin: 100, stepMax: 400 }, { loopMin: 1, loopMax: 1 });
                    break;
                case 7:
                    current = await page.scrollRandUpDownScoreRollBack(80, { delayMin: 500, delayMax: 4000 }, { stepMin: 200, stepMax: 450 }, { loopMin: 1, loopMax: 1 });
                    break;
                case 8:
                    current = await page.scrollRandUpDownScoreRollBack(85, { delayMin: 1000, delayMax: 4500 }, { stepMin: 70, stepMax: 550 }, { loopMin: 1, loopMax: 1 });
                    break;
            }
                ...
                if (randomScore(index)) {
                    if (linkActives.length > 0) {
                        try {
                            ...
                            await page.clickHref('a[href*="' + linkActives[random(0, linkActives.length - 1)].href + '"]');
