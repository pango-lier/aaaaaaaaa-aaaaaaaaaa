...
const processTargetDomain = async (page, link, level = 5, scoreRollDow = 90, config) => {
    try {
        ...
        for (let index = 0; index < random(100, 120); index++) {
            ...
            try {
                await page.moveRandom( random(400, 5000), random(422, 3000));
                ...
                switch (level) {
                    case 2:
                        current = await page.scrollRandUpDownScore(scoreRollDow, { delayMin: 200, delayMax: 500 }, { stepMin: 40, stepMax: 120 }, { loopMin: 2, loopMax: 4 });
                        break;
                    case 3:
                        current = await page.scrollRandUpDownScore(scoreRollDow, { delayMin: 200, delayMax: 500 }, { stepMin: 60, stepMax: 200 }, { loopMin: 1, loopMax: 3 });
                        break;
                    default:
                    case 4:
                        current = await page.scrollRandUpDownScore(scoreRollDow, { delayMin: 200, delayMax: 500 }, { stepMin: 40, stepMax: 300 }, { loopMin: 1, loopMax: 2 });
                        break;
                    case 5:
                        current = await page.scrollRandUpDownScore(scoreRollDow, { delayMin: 200, delayMax: 500 }, { stepMin: 70, stepMax: 350 }, { loopMin: 1, loopMax: 2 });
                        break;
                    case 6:
                        current = await page.scrollRandUpDownScore(scoreRollDow, { delayMin: 200, delayMax: 500 }, { stepMin: 100, stepMax: 400 }, { loopMin: 1, loopMax: 1 });
                        break;
                    case 7:
                        current = await page.scrollRandUpDownScore(scoreRollDow, { delayMin: 200, delayMax: 500 }, { stepMin: 150, stepMax: 450 }, { loopMin: 1, loopMax: 1 });
                        break;
                    case 8:
                        current = await page.scrollRandUpDownScore(scoreRollDow, { delayMin: 200, delayMax: 500 }, { stepMin: 70, stepMax: 550 }, { loopMin: 1, loopMax: 1 });
                        break;
                }
