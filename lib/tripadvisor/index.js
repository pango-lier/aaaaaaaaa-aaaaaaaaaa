      ...
      try {
        ...
        switch (step) {
          ...
          case "process":
            if (
              randomScore(
                config.rateClickOtherLink && maxChildClick > numberLinkClick
              )
            ) {
              ...
              if(!foundDomain){
                await processNegativeTargetDomain(
                page,
                link,
                config
              );
              await switchPageHref(
                page,
                "https://www.tripadvisor.com/Search?searchSessionId"
              );
              }
              numberLinkClick++;
            }
            ...
            if (foundDomain) step = "target-domain";
            break;
          case "target-domain":
            await processTargetDomain(
              page,
              domainExist,
              random(2, 8),
              90,
