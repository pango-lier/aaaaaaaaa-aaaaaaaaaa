...
const test = async () => {
  const browser = await puppeteer.launch({
    ...
    args: [
      "--no-sandbox",
      "--user-data-dir=/home/trong/.config/google-chrome/Profile 5",
      "--tz=Asia/Bangkok",
      "--no-first-run",
      "--no-sandbox",
      "--font-masking-mode=2",
  ...
  try {
    ...
    await googleLogin(pup,__dirname + "/helper/account-login.txt");
    await changePassword(pup, __dirname + "/helper/account-change-password.txt", "Abc@1234@@");
